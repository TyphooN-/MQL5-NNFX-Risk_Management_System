//+------------------------------------------------------------------+
//|                                ehlers_adaptive_bandpass_cube.mq5 |
...
#property description "Adaptive BandPass Cube - Ehlers Cycle Analytics"
                ...
                const int &spread[])
{
   ...
   for (int i = start; i < rates_total && !IsStopped(); i++)
   {
      ...
      if (g_peak != 0)
      {
         double normalized = bandPassBuf[i] / g_peak;
         result[i] = normalized * normalized * normalized; // cube
      }
      else
         result[i] = 0;
