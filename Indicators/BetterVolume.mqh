...
double AvgCalc[];
#else
// MQL4: 6 histogram buffers (one per color) + avg line + calc
double BufLowVol[];
double BufClimaxUp[];
double BufClimaxDn[];
double BufChurn[];
double BufClimaxChurn[];
double BufNormal[];
double AvgData[];
double AvgCalc[];
#endif
...
int OnInit()
{
   ...
   SetIndexBuffer(1, BufClimaxUp);
   SetIndexBuffer(2, BufClimaxDn);
   SetIndexBuffer(3, BufChurn);
   SetIndexBuffer(4, BufClimaxChurn);
   SetIndexBuffer(5, BufNormal);
   SetIndexBuffer(6, AvgData);
   SetIndexBuffer(7, AvgCalc);
   SetIndexStyle(0, DRAW_HISTOGRAM, STYLE_SOLID, 2, InpClrLowVol);
   SetIndexStyle(1, DRAW_HISTOGRAM, STYLE_SOLID, 2, InpClrClimaxUp);
   SetIndexStyle(2, DRAW_HISTOGRAM, STYLE_SOLID, 2, InpClrClimaxDn);
   ...
   SetIndexLabel(3, "Churn");
   SetIndexLabel(4, "Climax+Churn");
   SetIndexLabel(5, "Normal");
   SetIndexLabel(6, "AvgVol");
#endif
...
#else
   IndicatorShortName("BetterVol(" + IntegerToString(InpLookback) + ")");
   IndicatorDigits(0);
#endif
...
#else
   ArraySetAsSeries(BufLowVol, true);
   ArraySetAsSeries(BufClimaxUp, true);
   ArraySetAsSeries(BufClimaxDn, true);
   ArraySetAsSeries(BufChurn, true);
   ArraySetAsSeries(BufClimaxChurn, true);
   ArraySetAsSeries(BufNormal, true);
#endif
...
#else
      // Route volume to the matching color buffer, EMPTY_VALUE for others
      BufLowVol[pos]     = EMPTY_VALUE;
      BufClimaxUp[pos]   = EMPTY_VALUE;
      BufClimaxDn[pos]   = EMPTY_VALUE;
      BufChurn[pos]      = EMPTY_VALUE;
      BufClimaxChurn[pos]= EMPTY_VALUE;
      BufNormal[pos]     = EMPTY_VALUE;
      switch(cls)
      {
         case VOL_LOW:          BufLowVol[pos]     = vol; break;
         case VOL_CLIMAX_UP:    BufClimaxUp[pos]   = vol; break;
         case VOL_CLIMAX_DN:    BufClimaxDn[pos]   = vol; break;
         case VOL_CHURN:        BufChurn[pos]      = vol; break;
         case VOL_CLIMAX_CHURN: BufClimaxChurn[pos]= vol; break;
         default:               BufNormal[pos]     = vol; break;
      }
#endif
